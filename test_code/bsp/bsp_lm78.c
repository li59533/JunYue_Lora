 ...
 * @{  
 */
#define E32_POWER_ON	GPIO_WritePinOutput(GPIOC, 2, 1)
#define E32_POWER_OFF	GPIO_WritePinOutput(GPIOC, 2, 0)
/**
 * @}
 */
...
static void bsp_e32_halinit(void)
{
	...
	GPIO_WritePinOutput(GPIOC, 2, 0); // E32_Power
		...
		case LM78_STATUS_SEND_BYTES_REQ:
		{
			if(strstr((const char *)rev_buf,"OK") != 0 )
			{
				...
				else
				{
					...
					//NetTask_Send_Event(HAL_TASK_STANDBY_EVENT);
				}
